...
def get_profile_image_path(self, filename):
    return f'property_images/non-residential/maintenance/{self.pk}/{str(uuid.uuid4())}.png'


class NonResidentialMaintenance(models.Model):
    description = models.CharField(max_length=100)
    cost = models.FloatField()
    image = models.ImageField(max_length=255, upload_to=get_profile_image_path, null=True, blank=True)
    officer_property = models.ForeignKey('NonResidentialProperty', related_name='maintenance', on_delete=models.CASCADE)
