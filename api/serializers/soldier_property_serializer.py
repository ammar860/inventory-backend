...
class SoldierPropertySerializer(serializers.ModelSerializer):
    maintenance = SoldierMaintenanceSerializer(many=True, read_only=True)

    class Meta:
        model = SoldierProperty
        fields = (
            'id', 'name', 'qtr_no', 'loc', 'unit', 'image', 'maintenance', 'created_at', 'updated_at', 'created_by',
            'type', 'updated_by'
        )
        read_only_fields = (
